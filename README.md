...
This repo is **agent-agnostic, project-agnostic, kernel-free**. Generic rules live in `skills/`; specific projects are referenced via cliff notes in `docs/references/` (not vendored). A `catalog` skill helps pick what you need.
...
- **plan-iteration-gate** = convergent, *from approved spec*. Observable signal per step. Loops until one clean iteration. Trend shrinking → continue; recurring same scale → wrong altitude rescope.
...
docs/generic-principles.md      — cliff notes, no project specifics
...
- **code-structure** — helper extraction by theme, length signal ≤20 soft/40 hard, guard early return, should_/is_ predicate naming, minimal obvious fix
- **llm-tells** — marks, optimized+example, templated prose, orphan floaters, over-bulleting, double negatives
- **self-review-gate** — razor Gate1 (what's wrong? better way? cut test) + Gate2 iteration + resource lifecycle example, limit disclaimer
...
- **plan-iteration-gate** — iteration until converged, observable signal, human triggers
- **catalog** — picker that loads index+descs, asks 2-3 Qs, recommends subset + install checklist

All kernel-specific terminology lives only in `docs/references/kernel-style-as-example.md` as example of strict-project wrapper (ID-anchored rationale, token budgets, lint chain pattern). Core is kernel-free.

## References catalog (seed)

- `obra/superpowers` — source inspiration for plan/TDD/debugging (Apache-2.0, not vendored)
- `kernel-style-as-example` — how 4-phase+rationale+lint works there
